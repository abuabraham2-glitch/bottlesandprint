...
export function getAttachmentUrl(att: any): string {
  ...
  if (att.gmail_id && att.attachmentId) {
    return `https://bottlesandprint.app.n8n.cloud/webhook/download-attachment?messageId=${encodeURIComponent(att.gmail_id)}&attachmentId=${encodeURIComponent(att.attachmentId)}&mimeType=${encodeURIComponent(att.mimeType || "application/octet-stream")}&filename=${encodeURIComponent(att.name || "attachment")}`;
  }
