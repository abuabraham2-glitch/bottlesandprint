...
export default function Inbox() {
  ...
  const handleComposeSend = async () => {
    ...
    try {
      ...
      // Only insert the WAITING ON THEM record after webhook verification succeeds
      if (isBrandNew) {
        try {
      ...
      {isLoading ? (
        ...
        <div className="space-y-1">
          ...
          {displayedEmails.map(email => {
            ...
            return (
            <div key={email.id}
              className={`floating-card mb-0 cursor-pointer transition-colors ${anyUnread && mainTab === "needs_reply" ? "bg-foreground/5 hover:bg-foreground/10" : "hover:bg-muted/30"} ${selectedIds.has(email.id) ? "ring-2 ring-primary/50" : ""}`}
